...
public class printactualcurri {
    ...
    public void myMethod(String textFieldValue, String textFieldValue3, String textFieldValue4, String textFieldValue7, String textFieldValue8, String textFieldValue9) throws BadElementException, FileNotFoundException, DocumentException, IOException, SQLException {
        ...
        String outputFileName = textFieldValue + "-viewgrades2.pdf"; // concatenate the parameter value with ".pdf"
        ...
        Font totalboldblack = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLD, BaseColor.BLACK);
        ...
        PdfPageEventHelper pageEventHelper = new PdfPageEventHelper() {
            public void onStartPage(PdfWriter writer, Document document) {
                try {
                    ...
                    Chunk nameChunk = new Chunk("Name:                   " + (name), normalblack);
                    Chunk sexChunk = new Chunk("Sex: " + sex, normalblack);
                    ...
                    Paragraph header3 = new Paragraph("Student Number:   " + srcode, normalblack);
...
// Add an empty cell to align the Hours label with the Lec/Lab labels
        ...
        String sql = "SELECT * FROM studentfinalgrades WHERE  years = ? and semester = ?  and SR_Code='" + srcode + "' ORDER BY course_code"; // add an ORDER BY clause to sort the results by course code
...
// iterate over result set and add data to table
                    do {
                        ...
                        String gradeText = finalGrade.equals("0.00") ? "     " : finalGrade;
                        ...
                        PdfPCell subcell4 = new PdfPCell(new Phrase(rs.getString("unit"), normalblack));
                        subcell4.setBorder(com.lowagie.text.Rectangle.NO_BORDER);
                        subcell4.setHorizontalAlignment(Element.ALIGN_CENTER);

                        table2.addCell(subcell4);
        ...
        ResultSet rss = null;
        try {
            cons = DriverManager.getConnection("jdbc:mysql://localhost:3306/studentportal", "root", "");
            sts = (Statement) cons.createStatement();
            ResultSet rsss = sts.executeQuery("SELECT  FORMAT((SUM(SUBSTRING_INDEX(final_grade, ',', -1) * unit))/SUM(unit), 2) AS `Total GWA` FROM studentfinalgrades WHERE SR_Code = '" + srcode + "' AND Years BETWEEN 'First Year' AND 'Fourth Year' GROUP BY Program");
            if (rsss.next()) {
                String totalGWA = rsss.getString("Total GWA");
                Paragraph header78 = new Paragraph("Total GWA: " + totalGWA, totalboldblack);
                header78.setAlignment(Element.ALIGN_RIGHT);
                document.add(header78);
            }
        ...
        System.out.println("Grades per program generated successfully");
